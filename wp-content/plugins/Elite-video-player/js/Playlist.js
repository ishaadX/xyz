...
PLAYER.Playlist = function ($, video, options, mainContainer,element,/*ytWrapper,youtubePlayer,*/ preloader, myVideo, canPlay, click_ev, pw, hasTouch, deviceAgent, agentID, youtube_array) {
	...
	var youtubeControls = options.youtubeControls;
    ...
    function onPlayerStateChange(event) {
		...
		if(event.data === 1 && youtube_time==0 ) {
			...
			self.VIDEO.calculateYoutubeTotalTime(self.youtubePlayer.getDuration());
		}
        if(event.data === 1) {
			//playing
			...
			self._timer = setInterval(function() {
				...
				return;
				// //progress
				self.progressWidth = (self.youtubePlayer.getCurrentTime()/self.youtubePlayer.getDuration() )*self.VIDEO.videoTrack.width();
				self.VIDEO.videoTrackProgress.css("width", self.progressWidth);
				self.progressIdleWidth = (self.youtubePlayer.getCurrentTime()/self.youtubePlayer.getDuration() )*self.VIDEO.progressIdleTrack.width();
				self.VIDEO.progressIdle.css("width", self.progressIdleWidth);
				// //time
				self.VIDEO.calculateYoutubeElapsedTime(self.youtubePlayer.getCurrentTime());
				// //download
				self.buffered = self.youtubePlayer.getVideoLoadedFraction();
...
//            });
    ...
    $(options.videos).each(function loopingItems()
    {
		...
		if(self.VIDEO.onYouTubePlayerAPIReady){
			if(youtubeControls=="default controls")
			{
				if(self.options.posterImg=="")
				self.VIDEO.element.css("visibility","hidden");
			...
			else if(youtubeControls=="custom controls")
			{
				self.youtubePlayer = new YT.Player(document.getElementById('elite_vp_ytPlayer'), {
					height: '100%',
					width: '100%',
