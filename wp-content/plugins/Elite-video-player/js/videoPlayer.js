				...
				$.ajax({
					...
					success: function(data) {
						...
						/*$.each(data.feed.entry, function(i, item) {
							...
							var feedInfo = item.media$group.media$description.$t;
...
//                }
	...
	if(self._playlist.youtubePlayer!= undefined)
	{
		if(self.realFullscreenActive)
		{
			...
			self._playlist.youtubePlayer.setSize("100%","100%" );
		}
		else
		{
			switch(self.options.playlist){
...
Video.fn.resizeBars = function(){
	...
	{
		if(this._playlist.youtubePlayer!= undefined && this._playlist.youtubeSTARTED){
			//progress
			this.progressWidth = (this._playlist.youtubePlayer.getCurrentTime()/this._playlist.youtubePlayer.getDuration() )*this.videoTrack.width();
			this.videoTrackProgress.css("width", this.progressWidth);
			this.progressIdleWidth = (this._playlist.youtubePlayer.getCurrentTime()/this._playlist.youtubePlayer.getDuration() )*this.progressIdleTrack.width();
			this.progressIdle.css("width", this.progressIdleWidth);
			//download
			this.buffered = this._playlist.youtubePlayer.getVideoLoadedFraction();
...
Video.fn.pause = function()
{
	...
	{
		if(self._playlist.youtubeSTARTED)
			this.playButtonScreen.show();
		else
			this.playButtonScreen.hide();
	}
...
Video.fn.togglePlay = function()
{
  if (this.state == "elite_vp_playing")
  {
    this.pause();
	if(this._playlist.videos_array[this._playlist.videoid].videoType=="youtube" || this.options.videoType=="YouTube")
	this._playlist.youtubePlayer.pauseVideo();
  }
  else
  {
    this.play();
	if(this._playlist.videos_array[this._playlist.videoid].videoType=="youtube" || this.options.videoType=="YouTube")
	this._playlist.youtubePlayer.playVideo();
  }
};
  ...
  this.playBtnBg.append(this.playBtn);
    ...
    this.rewindBtn = $("<span />")
        ...
        .bind(self.CLICK_EV, function(){
			...
			if(self._playlist.youtubePlayer!= undefined){
			self._playlist.youtubePlayer.seekTo(0);
            self._playlist.youtubePlayer.playVideo();
			}
        });
    ...
    this.progressADBg.append(this.progressAD);
		...
		$(this.mainContainer).find(".elite_vp_playerElement").bind("mousemove mouseenter click", function(e){
			...
			if ($(this).hasClass("elite_vp_videoTrack")){
				...
				if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
				{
					self.toolTip.text(self.secondsFormat(self._playlist.youtubePlayer.getDuration()*perc));
				}
				else if(self._playlist.videos_array[self._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)")
		...
		this.videoTrack.bind("click",function(e){
			if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
			{
				...
				var perc = xPos / self.videoTrack.width();
				self._playlist.youtubePlayer.seekTo(self._playlist.youtubePlayer.getDuration()*perc);
			}
			else if(self._playlist.videos_array[self._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)")
			{
				var xPos = e.pageX - self.videoTrack.offset().left;
...
Video.fn.setupVolumeTrack = function()
{
    ...
    this.unmuteBtn.bind(this.CLICK_EV,$.proxy(function(){
        if(self.muted){
			...
			if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
				self._playlist.youtubePlayer.setVolume(volRatio*100);
			else if(self._playlist.videos_array[self._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)")
			...
			if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
				self._playlist.youtubePlayer.setVolume(0);
    ...
    self.volumeTrack.bind("mousedown",function(e){
		...
		if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
			self._playlist.youtubePlayer.setVolume(self.volPerc*100);
		else if(self._playlist.videos_array[self._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)")
        ...
        $(document).mousemove(function(e){
			...
			if(self._playlist.videos_array[self._playlist.videoid].videoType=="youtube" || self.options.videoType=="YouTube")
				self._playlist.youtubePlayer.setVolume((self.volumeTrackProgress.width()/self.volumeTrack.width())*100);
			else if(self._playlist.videos_array[self._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)")
